...
pub fn import_graph(path: impl AsRef<Path>, frame: &mut Frame) {
    ...
    for (k, v) in g.images.iter() {
        ...
        let img = frame.create_image_2d((width, height), vk::Format::R16g16b16a16Sfloat, 0);
    ...
    for (k, v) in g.tasks.iter() {
        let t = frame.create_task(k.clone(), |builder| {
            // color attachments
            for color_attachment in v.color_attachments.iter() {
                let out = {
                    let mut a = images
                        .get_mut(&color_attachment.id)
                        .expect(&format!("dependency not found: {}", color_attachment.id));
                    builder.color_attachment(color_attachment.index, a)
